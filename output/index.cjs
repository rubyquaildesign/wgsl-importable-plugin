...
function checkDuplicatedImports (path) {
  ...
  process$1.emitWarning(`'${path}' was included multiple times.`, {
 ...
 */
function loadChunks (source, path$1, extension, warn, root) {
  const unixPath = path$1.split(path.sep).join(path.posix.sep);
  ...
  let directory = path.dirname(unixPath);
  ...
  if (include.test(source)) {
    ...
    source = source.replace(include, (_, chunkPath) => {
      ...
      if (!chunkPath.indexOf('/')) {
        const base = process$1.cwd().split(path.sep).join(path.posix.sep);
      ...
      if (!path.extname(shader)) shader = `${shader}.${extension}`;

      const shaderPath = shader.split(path.sep).join(path.posix.sep);
      dependentChunks.get(unixPath)?.push(shaderPath);

      return loadChunks(
        fs.readFileSync(shader, 'utf8'),
        shader, extension,
        warn, root
      );
    });
...
exports.generateTypeScriptDefinition = generateTypeScriptDefinition;
exports.testImportAttributes = testImportAttributes;
exports.testImportModuleSpecifier = testImportModuleSpecifier;
